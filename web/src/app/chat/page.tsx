    ...
    <div className="flex flex-col h-[calc(100vh-6rem)] max-w-5xl mx-auto">
      ...
      >
        ...
        >
          ...
          <span className="text-sm text-emerald-400 font-medium">AI-Powered Predictions</span>
        </motion.div>
        <h1 className="text-4xl font-bold text-white mb-2 bg-gradient-to-r from-white via-zinc-200 to-zinc-400 bg-clip-text text-transparent">
          CrudeIntel AI Chat
        </h1>
      ...
      <Card className="flex-1 flex flex-col overflow-hidden border-zinc-800/50 bg-gradient-to-b from-zinc-900 to-zinc-950 shadow-2xl">
            ...
            {messages.map((message, index) => (
              ...
              >
                {message.role === 'assistant' && (
                  <motion.div
                    className="flex-shrink-0 w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center shadow-lg shadow-emerald-500/20"
                ...
                {message.role === 'user' && (
                  <motion.div
                    className="flex-shrink-0 w-10 h-10 rounded-xl bg-zinc-700 flex items-center justify-center"
                    whileHover={{ scale: 1.1, rotate: -5 }}
                  >
        ...
        <AnimatePresence>
          {messages.length <= 2 && (
            <motion.div
              ...
              className="px-6 pb-4"
            >
              <p className="text-xs text-zinc-500 mb-3 font-medium uppercase tracking-wide">Quick Questions</p>
                ...
                {SUGGESTED_QUESTIONS.map((q, i) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className="flex items-center gap-2 text-left text-sm px-4 py-3 rounded-xl bg-zinc-800/50 text-zinc-300 hover:bg-zinc-700/50 hover:text-white border border-zinc-700/50 transition-all"
        ...
        {/* Input Area */}
        <motion.div
          className="border-t border-zinc-800/50 p-4 bg-zinc-900/50"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex gap-3">
            <motion.div className="flex-1 relative" whileFocus={{ scale: 1.01 }}>
              <input
                ...
                className="w-full bg-zinc-800/50 border border-zinc-700/50 rounded-xl px-5 py-3.5 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-emerald-500/50 focus:ring-2 focus:ring-emerald-500/20 transition-all"
                disabled={isLoading}
              />
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                <kbd className="hidden sm:inline-block px-2 py-1 text-xs text-zinc-500 bg-zinc-700/50 rounded">
                  Enter
                </kbd>
              </div>
            </motion.div>
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button
                ...
                className="h-full px-5 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 rounded-xl shadow-lg shadow-emerald-500/20"
              >
                {isLoading ? (
